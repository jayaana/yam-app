...
(function () {
  ...
  function _bkInitRT() {
    ...
    // Vérification initiale : session partenaire déjà active ?
    (function _bkCheckInitialLobby() {
      var partnerRole = u.role === 'girl' ? 'boy' : 'girl';
      fetch(
        SB_URL + '/rest/v1/' + BK_SESSIONS_TBL +
        '?couple_id=eq.' + encodeURIComponent(u.couple_id) +
        '&created_by=eq.' + partnerRole +
        '&status=eq.active&select=id,created_by&limit=1',
        { headers: sb2Headers() }
      )
      .then(function(r){ return r.json(); })
      .then(function(rows){ _bkSetLobbyNav(!!(rows && rows.length)); })
      .catch(function(){});
    }());
      ...
      var chSessions = window._yamRT
        ...
        .on('postgres_changes', {
          ...
          if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
            ...
            var row = payload.new;
            if (!row) return;
            var sessionIsActive = row.status === 'active';
            _bkLibrary.forEach(function(book) {
              if (book.id === row.book_id) {
                book._syncSession = sessionIsActive ? row : null;
              }
            });
            // Pill nav : allumer si c'est le partenaire qui est hôte
            var _me = (typeof yamGetUser === 'function' && yamGetUser()) ? yamGetUser().role : null;
            var isPartnerSession = sessionIsActive && row.created_by && row.created_by !== _me;
            _bkSetLobbyNav(isPartnerSession);
            ...
            // Partenaire a terminé → éteindre pill nav
            _bkSetLobbyNav(false);
          }
  ...
  window.bkOpen = function () {
    ...
    haptic('light');
    _bkLoadLibrary();
    // Sync icônes thème au cas où le thème a changé depuis la dernière ouverture
    setTimeout(_bkSyncThemeIcons, 100);
  };
  ...
  function _bkStopSyncSession() {
    ...
    // Éteindre la pill nav quand on quitte la session
    _bkSetLobbyNav(false);
  }

  // ─── Pill "En sync" + signal navJeux vert ─────────────────────────
  // Appelé depuis le channel RT book_sessions dans _bkInitRT
  // et depuis _bkStopSyncSession.

  function _bkSetLobbyNav(active) {
    window._bkLobbyActive = !!active;
    // Pill sur la row "Lire ensemble"
    var pill = document.getElementById('bkLivePill');
    if (pill) pill.style.display = active ? 'inline-flex' : 'none';
    // Signal vers navJeux (couleur verte — arbitrée par _yamSyncNavJeux)
  ...
  function _bkCheckExistingSession(book) {
    ...
    sb2Fetch(BK_SESSIONS_TBL,
      ...
      toast.innerHTML =
        '<span style="font-size:13px;flex:1;">📡 <strong>' + escHtml(partnerName) + '</strong> lit en sync — rejoindre ?</span>' +
        '<div style="display:flex;gap:8px;flex-shrink:0;">' +
          '<button id="bkJoinYes" style="padding:5px 16px;border-radius:20px;background:#fff;' +
          'color:var(--accent);font-size:12px;font-weight:800;border:none;cursor:pointer;font-family:DM Sans,sans-serif;">Oui</button>' +
          '<button id="bkJoinNo" style="padding:5px 10px;border-radius:20px;background:rgba(255,255,255,.2);' +
  ...
  // Synchroniser l'icône soleil/lune des boutons thème books selon le thème actif
  function _bkSyncThemeIcons() {
    var isDark = document.documentElement.className === '' ||
                 document.body.classList.contains('dark') ||
                 document.documentElement.dataset.theme === 'dark';

    // Helper : montrer lune en dark, soleil en light
    function syncPair(moonId, sunId) {
      var moon = document.getElementById(moonId);
      var sun  = document.getElementById(sunId);
      if (!moon || !sun) return;
      if (isDark) {
    ...
    syncPair('bkThemeMoon', 'bkThemeSun');
    syncPair('bkRdrMoon',   'bkRdrSun');
  }
