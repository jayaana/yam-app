...
(function(){
  ...
  function _createEngine(cfg) {
    ...
    function upsertPresence(){
      ...
      if(!coupleId) return;
      var _presUid = yamGetUser ? yamGetUser().id : null;
      fetch(SB_URL+'/rest/v1/'+PRESENCE_TABLE+'?on_conflict=couple_id,role', {
        method:'POST',
    ...
    function deletePresence(){
      ...
      fetch(SB_URL+'/rest/v1/'+PRESENCE_TABLE+'?couple_id=eq.'+coupleId+'&role=eq.'+_me, {
        ...
        body: JSON.stringify({last_seen: staleTime})
      }).catch(function(){});
    ...
    function lobbyTick(){
      ...
      var coupleId = _getCoupleId();
      if(!coupleId) return;
      fetch(SB_URL+'/rest/v1/'+PRESENCE_TABLE+'?couple_id=eq.'+coupleId+'&select=role,last_seen', {headers:sb2Headers()})
      .then(function(r){return r.json();})
      .then(function(rows){
        if(_launched) return;
        ...
        rows.forEach(function(row){
          if(now - new Date(row.last_seen).getTime() < 10000){
            if(row.role==='girl') girlOk=true;
            if(row.role==='boy')  boyOk=true;
          }
        });
    ...
    function _startRTChannel(){
      ...
      _rtChannel = window._yamRT
        ...
        .subscribe(function(status){
          ...
          if (status === 'SUBSCRIBED') console.warn('[RT] ✅ game_state ' + _gameId + ' connecté — Realtime actif');
          else if (['CHANNEL_ERROR','TIMED_OUT','CLOSED'].indexOf(status) !== -1) console.warn('[RT] ⚠️ game_state ' + _gameId + ' ' + status + ' — fallback poll');
    ...
    function fetchState(){
      ...
      Promise.all([
        ...
        fetch(SB_URL+'/rest/v1/'+PRESENCE_TABLE+'?couple_id=eq.'+coupleId+'&select=role,last_seen', {headers:sb2Headers()}).then(function(r){return r.json();})
      ])
      .then(function(results){
        if(_saving) return;
        var rows    = results[0];
