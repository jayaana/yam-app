...
// Patch sbGet
var _origSbGet = (typeof sbGet !== 'undefined') ? sbGet : function(){return Promise.resolve([]);};
...
// Patch sbPost
var _origSbPost = (typeof sbPost !== 'undefined') ? sbPost : function(){return Promise.resolve({});};
...
// Patch sbPatch
var _origSbPatch = (typeof sbPatch !== 'undefined') ? sbPatch : function(){return Promise.resolve({});};
...
// Patch sbDelete
var _origSbDelete = (typeof sbDelete !== 'undefined') ? sbDelete : function(){return Promise.resolve({});};
...
(function(){
  ...
  'backdrop-filter:blur(6px);align-items:flex-end;justify-content:center;padding:env(safe-area-inset-top,44px) 0 0;">'+
  ...
  'max-height:calc(100dvh - env(safe-area-inset-top, 20px) - 10px);overflow-y:auto;overscroll-behavior:contain;touch-action:pan-y;-webkit-overflow-scrolling:touch;font-family:\'DM Sans\',sans-serif;transition:transform 0.3s cubic-bezier(.4,0,.2,1);" data-scrollable="1">' +
...
window.openAccountModal = function(){
  ...
  // Afficher/masquer section liaison partenaire
  var linkSec = document.getElementById('acLinkSection');
  if(linkSec){
    linkSec.style.display = u.partner_pseudo ? 'none' : '';
    var linkInput = document.getElementById('acLinkCode');
    if(linkInput) linkInput.value = '';
    var linkMsg = document.getElementById('acLinkMsg');
    if(linkMsg) linkMsg.textContent = '';
  }

  // Code couple - utiliser couple_code (court) ou couple_id (UUID)
  var code = u.couple_code || u.couple_id || '—';
  document.getElementById('acCoupleCode').textContent = code;
  // Afficher un hint si c'est un UUID (trop long pour partager facilement)
  var codeHint = document.querySelector('#acCoupleCode + small');
  if(!codeHint && code.length > 20){
    var hint = document.createElement('small');
    hint.style.cssText = 'display:block;font-size:10px;color:var(--muted);margin-top:4px;word-break:break-all;';
    hint.textContent = 'Copie et envoie ce code à ton partenaire';
    document.getElementById('acCoupleCode').after(hint);
  }
...
window.acSaveStartDate = function(){
  ...
  sb2Patch('v2_couples', 'id=eq.' + u.couple_id, { start_date: isoDate })
  .then(function(res){
    if(res && res.error){
      msg.textContent = '❌ Erreur : ' + (res.message || res.error);
    ...
    // Recharger la config couple pour rafraîchir tout
    loadCoupleConfig();
...
window.acChangePwd = function(){
  ...
  // RPC yam_change_password — vérifie ancien mdp et met à jour (hash auto-détecté côté SQL)
  fetch(SB2_URL + '/rest/v1/rpc/yam_change_password', {
    method: 'POST',
    headers: sb2Headers({'Content-Type':'application/json'}),
    body: JSON.stringify({ p_pseudo: u.pseudo, p_old_pwd: oldPwd, p_new_pwd: newPwd })
  })
  .then(function(r){ return r.json(); })
  .then(function(data){
    if(data && data.error){
...
// ── Lier un partenaire depuis Mon Compte ──
window.acLinkPartner = function(){
  ...
  fetch(SB2_URL + '/rest/v1/rpc/yam_join_couple', {
    method: 'POST',
    headers: sb2Headers({'Content-Type':'application/json'}),
    body: JSON.stringify({ p_user_id: u.id, p_couple_code: code })
  })
  .then(function(r){ return r.json(); })
  .then(function(data){
    if(data && data.error){
      ...
      msg.textContent = '✅ Compte lié avec succès !';
      msg.style.color = 'var(--green)';
      // Mettre à jour la session locale
      var s = v2LoadSession();
      if(s && s.user){
        s.user.couple_id = data.couple_id;
        s.user.partner_pseudo = data.partner_pseudo;
