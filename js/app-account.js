...
function _acSyncAvatarTopbar(url, role){
  // Avatar principal (profil connecté)
  ...
  // Avatar dans la bande humeur (accueil)
  var moodAvId = role === 'girl' ? 'yamMoodElleAvatar' : 'yamMoodLuiAvatar';
  var moodAvEl = document.getElementById(moodAvId);
  if(moodAvEl){
    var existingMoodImg = moodAvEl.querySelector('img.ac-avatar-img');
    if(!existingMoodImg){
      existingMoodImg = document.createElement('img');
      existingMoodImg.className = 'ac-avatar-img';
      existingMoodImg.style.cssText = 'width:100%;height:100%;object-fit:cover;border-radius:50%;position:absolute;inset:0;';
      moodAvEl.style.position = 'relative';
      moodAvEl.style.overflow = 'hidden';
      moodAvEl.appendChild(existingMoodImg);
    }
    existingMoodImg.src = url;
    existingMoodImg.style.display = '';
  }
}

// ✅ FIX — Synchronise l'avatar du partenaire sur tous les éléments visuels
function _acSyncPartnerAvatar(url, role){
  // Avatar humeur partenaire (accueil)
  var moodAvId = role === 'girl' ? 'yamMoodElleAvatar' : 'yamMoodLuiAvatar';
  var moodAvEl = document.getElementById(moodAvId);
  if(moodAvEl){
    var existingMoodImg = moodAvEl.querySelector('img.ac-avatar-img');
  ...
  // Avatar partenaire dans les bulles de messages et typing indicator
  var dmClass = role === 'girl' ? '.dm-avatar-girl' : '.dm-avatar-boy';
  document.querySelectorAll(dmClass).forEach(function(el){
    var img = el.querySelector('img');
    if(img) img.src = url;
  });
  // Avatar dans le header InstaLove (dmHeaderAvatarOther)
  var profile = (typeof getProfile === 'function') ? getProfile() : null;
  if(profile && profile !== role){
    var otherEl = document.getElementById('dmHeaderAvatarOther');
    if(otherEl){
      var oImg = otherEl.querySelector('img');
      if(!oImg){ oImg = document.createElement('img'); oImg.style.cssText='width:100%;height:100%;object-fit:cover;border-radius:50%;'; otherEl.appendChild(oImg); }
      oImg.src = url;
    }
  }
  // Stocké pour usage dans showMsgHeaderPill (notif pilule)
  window._yamPartnerAvatarUrl = url;
  window._yamPartnerAvatarRole = role;
}

// ✅ FIX — Charge l'avatar du partenaire depuis Supabase Storage
window._acLoadPartnerAvatar = function(){
  ...
  .then(function(rows){
    ...
    var probe = new Image();
    probe.onload = function(){ _acSyncPartnerAvatar(url, partner.role); };
    probe.src = url;
  })
  .catch(function(){});
};
...
window._acLoadAvatarTopbarOnStart = function(u){
  ...
  var probe = new Image();
  probe.onload = function(){ _acSyncAvatarTopbar(url, u.role); };
  probe.src = url;
  // ✅ FIX — Charger aussi l'avatar du partenaire
  setTimeout(function(){ if(window._acLoadPartnerAvatar) window._acLoadPartnerAvatar(); }, 500);
};
