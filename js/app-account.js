...
(function(){
  ...
  'max-height:92vh;overflow-y:auto;overscroll-behavior:contain;touch-action:pan-y;-webkit-overflow-scrolling:touch;font-family:\'DM Sans\',sans-serif;transition:transform 0.3s cubic-bezier(.4,0,.2,1);">' +
...
window.acChangePwd = function(){
  ...
  // Appel RPC change_password (vérifie ancien hash + met à jour)
  // On hash côté JS avec SHA-256 simple puis on passe à la RPC
  function simpleHash(str){
    // Utilise l'API SubtleCrypto si dispo, sinon encode basique
    return crypto.subtle
      ...
      : Promise.resolve(btoa(unescape(encodeURIComponent(str))));
  }

  Promise.all([simpleHash(oldPwd), simpleHash(newPwd)])
  .then(function(hashes){
    return fetch(SB2_URL + '/rest/v1/rpc/change_password', {
      method: 'POST',
      headers: sb2Headers({'Content-Type':'application/json'}),
      body: JSON.stringify({ p_pseudo: u.pseudo, p_old_hash: hashes[0], p_new_hash: hashes[1] })
    }).then(function(r){ return r.json(); });
  })
  .then(function(data){
    if(data && data.error){
...
window.acLinkPartner = function(){
  ...
  fetch(SB2_URL + '/rest/v1/rpc/join_couple_by_code', {
    method: 'POST',
    headers: sb2Headers({'Content-Type':'application/json'}),
