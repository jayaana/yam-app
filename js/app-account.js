...
(function(){
  ...
  // Anti-boucle : consommé immédiatement, bloque un seul cycle de poll
...
// ═════════════════════════════════════════════════════════════════
// yamToggleAccountModal — Toggle paramètres
...
// Patch closeAccountModal : revenir sur l'accueil après fermeture des paramètres
// ← migré depuis app-inline.js BLOC 9
(function() {
  function patchClose() {
    if (typeof window.closeAccountModal !== 'function') { setTimeout(patchClose, 200); return; }
    if (window._yamCloseAccountPatched) return;
    window._yamCloseAccountPatched = true;
...
// ═════════════════════════════════════════════════════════════════
// Avatar sync topbar — init + hook setProfile
// ← migré depuis app-inline.js BLOC 13
// ═════════════════════════════════════════════════════════════════
(function(){
  ...
  // Patch _acSyncAvatarTopbar (défini plus haut dans ce fichier) pour rafraîchir
  // les avatars par défaut après chaque changement de photo de profil
  (function patchAcSync(){
    if(!window._acSyncAvatarTopbar){setTimeout(patchAcSync,200);return;}
    if(window._yamAcSyncPatched) return;
    window._yamAcSyncPatched=true;
    ...
    window._acSyncAvatarTopbar=function(profile,avatarUrl){
      ...
      if(avEl&&avEl.tagName==='IMG'&&!avatarUrl) avEl.src=window.yamAvatarSrc(profile||getProfile()||'girl');
      initDefaultAvatars();
    };
  })();

  if(document.readyState==='loading'){ document.addEventListener('DOMContentLoaded',function(){initDefaultAvatars();}); }
  else { initDefaultAvatars(); }

  // Hook setProfile pour re-sync avatars au changement de profil
