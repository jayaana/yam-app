...
function _v2AfterLogin(result, msgId){
  ...
  // Déclencher l'init app immédiatement
  var u = v2GetUser();
  if(u){
  ...
  // Bloquer brièvement les événements pointer pour éviter les "ghost touch"
  // (le doigt qui lâche "Se connecter" peut déclencher un clic sur la nav)
  document.body.style.pointerEvents = 'none';
  window.scrollTo({ top: 0, behavior: 'instant' });

  setTimeout(function(){
    document.body.style.pointerEvents = '';
    // Naviguer vers l'onglet home directement (plus fiable que .click())
