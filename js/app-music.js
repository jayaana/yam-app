...
function savePlays(file){
  ...
  .then(function(r){
    if(!r.ok) return;
    return fetch(SB_URL + '/rest/v1/song_plays?song_key=eq.' + encodeURIComponent(file) + '&couple_id=eq.' + _spCoupleId + '&select=play_count', { headers: sb2Headers() });
  })
...
function loadFavorites(){
  ...
  return sb2Fetch('favorites','couple_id=eq.'+_sfId+'&select=profile,song_file')
