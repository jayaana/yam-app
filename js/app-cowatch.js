// ════════════════════════════════════════════════════════════════════
// APP-COWATCH.JS v3.0-beta
// Nouveautés : présence, chat texte, réactions séparées, sync améliorée
// SQL : ALTER TABLE v2_cowatch_sessions ADD COLUMN IF NOT EXISTS yt_id text;
...
//       ALTER TABLE v2_cowatch_sessions ADD COLUMN IF NOT EXISTS chat jsonb DEFAULT '[]';
//       ALTER TABLE v2_cowatch_sessions ADD COLUMN IF NOT EXISTS presence jsonb DEFAULT '{}';
// ════════════════════════════════════════════════════════════════════
(function () {
  'use strict';
  ...
  var POLL_MS  = 1800;   // poll sync
  var PRES_MS  = 4000;   // heartbeat présence
  var SYNC_IV  = 12000;  // resync forcée toutes les 12s

  var _myRole=null,_coupleId=null,_sessionId=null;
  var _isHost=false,_player=null,_ytReady=false;
  var _pollIv=null,_timeIv=null,_presIv=null,_syncIv=null;
  var _isSyncing=false,_lastTs=0,_lastChatTs=0,_lastReactTs=0;
  ...
  st.textContent=[
    ...
    '.cw-cbtn.dis{opacity:.3;pointer-events:none;}',
    '.cw-time{flex:1;font-size:11px;color:var(--muted);font-variant-numeric:tabular-nums;font-weight:600;text-align:center;}',
    // Barre présence
    '.cw-pres-bar{flex-shrink:0;padding:7px 16px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--border);background:var(--s1);}',
    '.cw-pres-item{display:flex;align-items:center;gap:6px;font-size:12px;color:var(--muted);font-weight:500;}',
    ...
    '.cw-hbadge{margin-left:auto;font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:3px 8px;border-radius:10px;background:rgba(201,120,96,.15);color:var(--accent);border:1px solid rgba(201,120,96,.3);}',
    // Réactions
    ...
    // Réaction flottante
    '.cw-float{position:fixed;pointer-events:none;z-index:2700;animation:cwFloat 1.5s ease-out forwards;display:flex;flex-direction:column;align-items:center;gap:2px;}',
    '.cw-float-emoji{font-size:30px;line-height:1;}',
    '.cw-float-name{font-size:10px;font-weight:700;color:#fff;text-shadow:0 1px 4px rgba(0,0,0,.6);white-space:nowrap;}',
    ...
    '.cw-cavatar{width:24px;height:24px;border-radius:50%;background:var(--s3);display:flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;font-weight:700;color:var(--muted);}',
  ...
  document.body.insertAdjacentHTML('beforeend',
    ...
    '<div id="cwScBeta" class="cw-sc">' +
      ...
      '<button class="cw-btn" onclick="window._cwBetaOk()">' +
        'Acc\u00e9der \u2728' +
      '</button>' +
    '</div>' +
    // Lobby
    ...
    '<div id="cwScPlayer" class="cw-sc">' +
      // Présence
      '<div class="cw-pres-bar">' +
        '<div class="cw-pres-item"><div class="cw-pdot" id="cwPresMe"></div><span id="cwPresNameMe">Moi</span></div>' +
        '<div class="cw-pres-item"><div class="cw-pdot" id="cwPresOther"></div><span id="cwPresNameOther">L\u2019autre</span></div>' +
  ...
  function _sc(id) {
    ['cwScBeta','cwScLobby','cwScWait','cwScJoin','cwScPlayer'].forEach(function(s) {
      var el=document.getElementById(s); if(el) el.classList.toggle('on',s===id);
    });
  }
  function _ytId(url) {
  ...
  function _fmt(s) {
    s=Math.floor(s||0); var m=Math.floor(s/60),ss=s%60;
    return m+':'+(ss<10?'0':'')+ss;
  }
  function _getCoupleId() {
    var u=typeof v2GetUser==='function'?v2GetUser():null; return u?u.couple_id:null;
  }
  ...
  function _initial(r){return _name(r).charAt(0).toUpperCase();}
  function _escHtml(s){
    return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
  }
  function _isEmojiOnly(s) { return /^(\p{Emoji_Presentation}|\p{Extended_Pictographic})+$/u.test(s.trim()); }
  ...
  window._cwLaunch=function() {
    ...
    .then(function(){
      return fetch(SB2_URL+'/rest/v1/'+TABLE,{
        ...
        body:JSON.stringify({
          couple_id:_coupleId, yt_id:ytId, host_role:_myRole, active:true,
  ...
  function _startPlayer(ytId) {
    ...
    if(badge) badge.style.display=_isHost?'':'none';
    // Contrôles seek : désactivés pour non-hôte
    ['cwBack10','cwFwd10','cwPlayBtn'].forEach(function(id){
      var el=document.getElementById(id);
      if(el) el.classList.toggle('dis',!_isHost);
    });
    // Noms présence
    ...
    _startSyncPoll();
    _startPresence();
    // Resync forcée toutes les SYNC_IV ms (non-hôte seulement)
    if(!_isHost) {
      _syncIv=setInterval(function(){_forcedResync();},SYNC_IV);
    }
  }
  ...
  function _applyState(state) {
    ...
    var target=(state.currentTime||0)+(state.playing?lag:0);
    _player.seekTo(target,true);
    setTimeout(function(){
  ...
  // Resync forcée : récupère le temps courant de l'hôte et se cale dessus
  function _forcedResync() {
    if(!_sessionId||_isSyncing) return;
    ...
    .then(function(rows){
      ...
      if(!state.ts) return;
      // Force la resync même si le ts est le même
      _lastTs=0;
      _applyState(state);
    }).catch(function(){});
  }
  ...
  function _startPresence() {
    ...
    _heartbeat();
    _presIv=setInterval(_heartbeat,PRES_MS);
  }

  function _heartbeat() {
    if(!_sessionId) return;
    // Lire presence courante, mettre à jour mon slot
    fetch(SB2_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId)+'&select=presence',{headers:sb2Headers()})
    .then(function(r){return r.json();})
    .then(function(rows){
      if(!rows||!rows.length) return;
      var pres=rows[0].presence||{};
      pres[_myRole]=Date.now();
      fetch(SB2_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId),{
        method:'PATCH',
        headers:sb2Headers({'Content-Type':'application/json','Prefer':'return=minimal'}),
  ...
  function _updatePresenceUI(pres) {
    ...
    var me=document.getElementById('cwPresMe');
    var ot=document.getElementById('cwPresOther');
  ...
  function _showFloat(emoji,role) {
    ...
    var x=(isMine?15:55)+Math.random()*25;
    el.style.cssText='left:'+x+'vw;bottom:40vh;animation-duration:'+(1.1+Math.random()*0.4)+'s;';
    el.innerHTML='<div class="cw-float-emoji">'+emoji+'</div><div class="cw-float-name">'+_escHtml(_name(role))+'</div>';
  ...
  function _applyChat(msgs) {
    ...
    // Trouver les nouveaux messages (ts > _lastChatTs et role != moi)
    msgs.forEach(function(msg){
      if(msg.ts<=_lastChatTs) return;
      if(msg.role===_myRole) return; // déjà affiché localement
  ...
  function _addChatMsg(msg,isMine) {
    ...
    var empty=document.getElementById('cwChatEmpty');
    ...
    var name=_name(msg.role||_myRole);
    var init=_initial(msg.role||_myRole);
    ...
    if(msg.type==='react') {
      ...
      content=_escHtml(msg.text||'');
      if(_isEmojiOnly(msg.text||'')) bubbleClass+=' emoji-only';
    }
    d.innerHTML=
      '<div class="cw-cavatar">'+_escHtml(init)+'</div>'+
      '<div class="cw-cbody">'+
        (isMine?'':'<div class="cw-cname">'+_escHtml(name)+'</div>')+
        '<div class="'+bubbleClass+'">'+content+'</div>'+
        '<div class="cw-ctime">'+t+'</div>'+
      '</div>';
    chat.appendChild(d);
    chat.scrollTop=chat.scrollHeight;
    // Mettre à jour _lastChatTs si c'est mon message
  ...
  function _stopAll() {
    ...
    _isHost=false;_isSyncing=false;_lastTs=0;_lastChatTs=0;_lastReactTs=0;_sessionId=null;
    ...
    var ch=document.getElementById('cwChat');
    if(ch)ch.innerHTML='<div class="cw-chat-empty" id="cwChatEmpty">Aucun message pour l\u2019instant \uD83C\uDF7F</div>';
