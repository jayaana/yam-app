...
(function () {
  ...
  var TABLE_PL = 'cowatch_playlist'; // playlist persistante par couple
  ...
  document.body.insertAdjacentHTML('beforeend',
    ...
    '<div id="cwHdr">' +
      ...
      '<button id="cwHdrThemeBtn" class="dm-topbar-theme" title="Changer le th\u00e8me">' +
        '<svg class="gvh-moon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3a7 7 0 0 0 9.79 9.79z"/></svg>' +
        '<svg class="gvh-sun" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:none"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>' +
      '</button>' +
    ...
    '<div id="cwScLobby" class="cw-sc">' +
      ...
      '<div id="cwLobbyNewLink">' +
        ...
        '<div class="cw-tabs">' +
          '<div class="cw-tab active" id="cwTabLink">\uD83D\uDD17 Coller un lien</div>' +
          '<div class="cw-tab" id="cwTabSearch">\uD83D\uDD0D Rechercher</div>' +
        '</div>' +
        '<div id="cwPanelLink">' +
        ...
        '<div id="cwPanelSearch" style="display:none;">' +
          '<div class="cw-search-wrap">' +
            ...
            '<button class="cw-search-clear" id="cwSearchClear"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>' +
          '</div>' +
          '<div class="cw-search-results" id="cwSearchResults"></div>' +
        '</div>' +
        '</div>' +
        '<button class="cw-btn" id="cwGoBtn" disabled>Lancer ce lien \uD83C\uDFAC</button>' +
      '</div>' +
      // Séparateur
      ...
      '<div id="cwLobbyPl">' +
        '<div class="cw-lpl-box">' +
          '<div class="cw-lpl-hdr">' +
            ...
            '<div class="cw-lpl-eye" id="cwLplEye">' +
          ...
          '<div class="cw-lpl-add" id="cwLplAddRow" style="display:none;">' +
            ...
            '<button class="cw-lpl-add-btn" id="cwLplAddBtn" disabled>' +
          ...
          '</div>' +
        '</div>' +
        '<button class="cw-btn-pl" id="cwGoPlBtn" disabled>Lancer la playlist \u25b6</button>' +
      '</div>' +
    ...
    '<div id="cwScWait" class="cw-sc">' +
      ...
      '<button class="cw-btn-ghost">Annuler la session</button>' +
    '</div>' +
    '<div id="cwScJoin" class="cw-sc">' +
      ...
      '<button class="cw-btn">Rejoindre \uD83C\uDF7F</button>' +
      '<button class="cw-btn-ghost" style="margin-top:4px;">Pas maintenant</button>' +
    '</div>' +
    '<div id="cwScPlayer" class="cw-sc">' +
      ...
      '<div class="cw-top-bar">' +
        ...
        '<div class="cw-top-actions">' +
          // Plein écran
          '<div class="cw-icon-btn" id="cwFsBtn" title="Plein \u00e9cran">' +
          ...
          '<button id="cwCoCtlBtn" class="cw-coctl-btn" style="display:none;">'+
      ...
      '</div>' +
      '<div class="cw-ctrl">' +
        '<div class="cw-cbtn pri" id="cwPlayBtn">' +
        ...
        '</div>' +
        '<div class="cw-cbtn dis" id="cwPrevBtn" title="Vid\u00e9o pr\u00e9c\u00e9dente">' +
        ...
        '<div class="cw-cbtn dis" id="cwSkipBtn" title="Vid\u00e9o suivante">' +
        ...
        '</div>' +
        '<div class="cw-cbtn" id="cwPlBtn" title="Playlist">' +
          '\uD83C\uDFAC' +
        '</div>' +
      '</div>' +
      // Réactions (masquées quand playlist ouverte)
      '<div class="cw-reacts" id="cwReacts">' +
        '<button class="cw-rbtn">\u2764\uFE0F</button>' +
        '<button class="cw-rbtn">\uD83D\uDE02</button>' +
        '<button class="cw-rbtn">\uD83D\uDE2E</button>' +
        '<button class="cw-rbtn">\uD83D\uDD25</button>' +
        '<button class="cw-rbtn">\uD83D\uDC4F</button>' +
        '<button class="cw-rbtn">\uD83D\uDE2D</button>' +
      ...
      '<div class="cw-chat-wrap" id="cwChatWrap">' +
        ...
        '<div class="cw-chat-input-wrap">' +
          ...
          '<button class="cw-send-btn" id="cwSendBtn" disabled>' +
      ...
      '<div class="cw-pl-panel" id="cwPlPanel">' +
        ...
        '<div class="cw-pl-add" id="cwPlAddRow">' +
          ...
          '<button class="cw-pl-add-btn" id="cwPlAddBtn" disabled>' +
  ...
  /* ── Event delegation cowatch — remplace les onclick= retirés (#89 CSP) ── */
  (function(){
    var ov = document.getElementById('cwOv');
    if(!ov) return;
    ov.addEventListener('click', function(e){
      var t = e.target;
      var closest = t.closest.bind(t);
      // Fermer modal
      if(closest('.cw-back'))           { window.closeCowatchModal && window.closeCowatchModal(); return; }
      // Thème
      if(t.id === 'cwHdrThemeBtn' || closest('#cwHdrThemeBtn')) { window._cwToggleTheme && window._cwToggleTheme(); return; }
      // Tabs
      if(t.id === 'cwTabLink')   { window._cwSwitchTab && window._cwSwitchTab(0); return; }
      if(t.id === 'cwTabSearch') { window._cwSwitchTab && window._cwSwitchTab(1); return; }
      // Recherche
      if(t.id === 'cwSearchClear' || closest('#cwSearchClear')) { window._cwSearchClear && window._cwSearchClear(); return; }
      // Lancer lien
      if(t.id === 'cwGoBtn')   { window._cwLaunch && window._cwLaunch(); return; }
      // Playlist locale toggle
      if(t.id === 'cwLplEye' || closest('#cwLplEye')) { window._cwLplToggle && window._cwLplToggle(); return; }
      // Ajouter/vider lpl
      if(t.id === 'cwLplAddBtn') { window._cwLplAddOrClear && window._cwLplAddOrClear(); return; }
      // Lancer playlist
      if(t.id === 'cwGoPlBtn')   { window._cwLaunchPlaylist && window._cwLaunchPlaylist(); return; }
      // Annuler session
      if(closest('.cw-btn-ghost') && t.textContent.indexOf('Annuler') !== -1) { window._cwCancel && window._cwCancel(); return; }
      // Rejoindre
      ...
      // Contrôles player
      if(t.id === 'cwPlayBtn'  || closest('#cwPlayBtn'))  { window._cwPlay    && window._cwPlay();    return; }
      if(t.id === 'cwBack10'   || closest('#cwBack10'))   { window._cwBack10  && window._cwBack10();  return; }
      if(t.id === 'cwPrevBtn'  || closest('#cwPrevBtn'))  { window._cwPrev    && window._cwPrev();    return; }
      if(t.id === 'cwSkipBtn'  || closest('#cwSkipBtn'))  { window._cwSkip    && window._cwSkip();    return; }
      if(t.id === 'cwFwd10'    || closest('#cwFwd10'))    { window._cwFwd10   && window._cwFwd10();   return; }
      if(t.id === 'cwPlBtn'    || closest('#cwPlBtn'))    { window._cwPlToggle && window._cwPlToggle(); return; }
      // Réactions
      if(closest('.cw-rbtn')) { window._cwReact && window._cwReact(t.textContent.trim()); return; }
      // Envoyer chat
      if(t.id === 'cwSendBtn') { window._cwSend && window._cwSend(); return; }
      // Playlist panel toggle (close btn)
      if(closest('.cw-pl-close-btn')) { window._cwPlToggle && window._cwPlToggle(); return; }
      // Importer playlist sauvegardée
      if(t.id === 'cwPlImportBtn' || closest('#cwPlImportBtn')) { window._cwImportSavedPl && window._cwImportSavedPl(); return; }
      // Ajouter/vider playlist active
      if(t.id === 'cwPlAddBtn') { window._cwPlAddOrClear && window._cwPlAddOrClear(); return; }
      // Supprimer de lpl (data-lpl-rm)
      var lplRmBtn = closest('.cw-lpl-rm[data-lpl-idx]');
      if(lplRmBtn) { window._cwLplRemove && window._cwLplRemove(parseInt(lplRmBtn.dataset.lplIdx)); return; }
      // Supprimer de playlist active (data-pl-rm)
      var plRmBtn = closest('.cw-pl-rm[data-pl-idx]');
      if(plRmBtn) { window._cwPlRemove && window._cwPlRemove(parseInt(plRmBtn.dataset.plIdx)); return; }
      // Sauter à une vidéo dans playlist (data-pl-jump)
      var plJump = closest('[data-pl-jump]');
      if(plJump && !plRmBtn) { window._cwPlJump && window._cwPlJump(parseInt(plJump.dataset.plJump)); return; }
    });
  })();


  // ── Helpers ────────────────────────────────────────────────────────
  function _sc(id){['cwScLobby','cwScWait','cwScJoin','cwScPlayer'].forEach(function(s){var el=document.getElementById(s);if(el)el.classList.toggle('on',s===id);});}
  ...
  function _loadSavedPlaylist(cb){
    if(!_coupleId)return;
    fetch(SB_URL+'/rest/v1/'+TABLE_PL+'?couple_id=eq.'+encodeURIComponent(_coupleId)+'&order=position.asc',{headers:sb2Headers()})
  ...
  window._cwLplRemove=function(idx){
    ...
    _cwConfirm('Supprimer cette vidéo de la playlist ?',function(){
      fetch(SB_URL+'/rest/v1/'+TABLE_PL+'?id=eq.'+encodeURIComponent(item.id),{
  ...
  function _startWaitPoll(){
    ...
    _pollIv=setInterval(function(){
      ...
      fetch(SB_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId)+'&select=state,yt_id,playlist,playlist_index',{headers:sb2Headers()})
      .then(function(r){return r.json();})
      .then(function(rows){
        if(!rows||!rows.length)return;
  ...
  window._cwCancel=function(){
    _stopPoll();
    if(_sessionId){fetch(SB_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId),{
      method:'PATCH',headers:sb2Headers({'Content-Type':'application/json','Prefer':'return=minimal'}),
      body:JSON.stringify({active:false})
    }).catch(function(){});}
  ...
  function _startSyncPoll(){
    ...
    _pollIv=setInterval(function(){
      if(!_sessionId)return;
      fetch(SB_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId)+'&select=state,active,chat,presence,playlist,playlist_index',{headers:sb2Headers()})
      .then(function(r){return r.json();})
      .then(function(rows){
        if(!rows||!rows.length)return;
        var row=rows[0];
      ...
      if(removedYtId){
        ...
        if(savedIdx!==-1){
          ...
          if(savedItem&&savedItem.id){
            fetch(SB_URL+'/rest/v1/'+TABLE_PL+'?id=eq.'+encodeURIComponent(savedItem.id),{
              method:'DELETE',headers:sb2Headers({'Prefer':'return=minimal'})
            }).catch(function(){});
          }
        }
  ...
  function _renderPlaylist(){
    ...
    list.innerHTML=_playlist.map(function(item,i){
      ...
      var rmHtml=(_isHost&&!isCurrent&&!isPast)
        ?'<button class="cw-pl-rm" data-pl-idx="'+i+'">' +
  ...
  function _heartbeat(){
    if(!_sessionId)return;
    fetch(SB_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId)+'&select=presence',{headers:sb2Headers()})
    .then(function(r){return r.json();})
    .then(function(rows){
      if(!rows||!rows.length)return;
  ...
  window._cwSend=function(){
    ...
    fetch(SB_URL+'/rest/v1/'+TABLE+'?id=eq.'+encodeURIComponent(_sessionId)+'&select=chat',{headers:sb2Headers()})
    .then(function(r){return r.json();})
    .then(function(rows){
      if(!rows||!rows[0])return;
  ...
  window._cwToggleTheme=function(){
    /* Délègue au système global — évite la désynchronisation lune/soleil */
    if (window.applyThemeToggle) window.applyThemeToggle();
  };
  ...
  function _stopAll(){
    ...
    _playlist=[];_plIndex=0;_currentYtId=null;_plOpen=false;_savedPlaylist=[];_launchedFromLink=false;_coControl=false;_lastCmdTs=0;
...
// ═══════════════════════════════════════════════════════════════════
// Cowatch live pill — Realtime INSERT/UPDATE + fallback poll 5s
// ← migré depuis app-inline.js BLOC 14
// Surveille cowatch_sessions pour afficher la pill "En direct" et
// le glow navJeux. _yamSyncNavJeux tient compte aussi de _jxLobbyActive
// (lobbies jeux multi) pour ne pas écraser son état.
// ═══════════════════════════════════════════════════════════════════
(function(){
  var TABLE      = 'cowatch_sessions';
  var _liveIv    = null;
  ...
  function _applyLiveState(show){
    ...
    if(typeof window._yamSyncNavJeux === 'function') window._yamSyncNavJeux();
  }
  ...
  function _initCwLive(){
    ...
    _closeRT();
    _stopFallbackPoll();
    if(window._yamRT) _subscribeRT(u.couple_id);
    else _startFallbackPoll();
  }

  document.addEventListener('yam:rt_ready',      function(){ _initCwLive(); });
  document.addEventListener('yam:session_ready', function(){
    setTimeout(function(){ if(!_rtChannel && !_liveIv) _initCwLive(); }, 800);
  });

  window._cwLiveCheck = _checkLive;

  // Sync navJeux couleurs lobby : rose=cowatch, doré=jeux multi, vert=co-lecture
  // ← migré depuis app-inline.js BLOC 14
  window._yamSyncNavJeux = function(){
    var navJeux = document.getElementById('navJeux');
    if(!navJeux) return;
    var onJeuxTab = window._currentTab === 'jeux';
    var cwActive = !!(window._cwLobbyActive)  && !onJeuxTab;
    var jxActive = !!(window._jxLobbyActive)  && !onJeuxTab;
    var bkActive = !!(window._bkLobbyActive)  && !onJeuxTab;
    var anyActive = cwActive || jxActive || bkActive;
    // Classe générique conservée pour compat
    navJeux.classList.toggle('partner-in-lobby', anyActive);
    // Priorité : cowatch (rose) > jeux multi (doré) > livres (vert)
    navJeux.classList.toggle('partner-in-lobby--pink',  cwActive);
    navJeux.classList.toggle('partner-in-lobby--gold',  !cwActive && jxActive);
    navJeux.classList.toggle('partner-in-lobby--green', !cwActive && !jxActive && bkActive);
  };

  // Sync au changement d'onglet via yamSwitchTab
  // ← intégré depuis app-inline.js BLOC 14 — remplace le patch yamSwitchTab
  // par un écouteur d'événement custom dispatché depuis app-nav.js
  // Note : yamSwitchTab expose window._currentTab, donc _yamSyncNavJeux
  // peut lire directement l'onglet courant sans patch supplémentaire.
  document.addEventListener('yam:tab_switched', function(e){
