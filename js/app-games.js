...
(function(){
  ...
  function enterLobby(){
    ...
    Promise.all([
      fetch(SB2_URL+'/rest/v1/'+SKYJO_TABLE+'?status=eq.playing&order=created_at.desc&limit=1&select=id,status,state,created_by',{headers:sb2Headers()}).then(function(r){return r.json();}),
      fetch(SB2_URL+'/rest/v1/'+SKYJO_PRESENCE+'?select=profile',{headers:sb2Headers()}).then(function(r){return r.json();})
    ])
  ...
  function fetchState(){
    ...
    Promise.all([
      ...
      fetch(SB2_URL+'/rest/v1/'+SKYJO_PRESENCE+'?select=profile,updated_at',{headers:sb2Headers()}).then(function(r){return r.json();})
    ])
    .then(function(results){
      if(_saving) return;
      var rows     = results[0];
