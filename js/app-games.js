// ═══════════════════════════════════════════════════════════
// app-games.js — Quiz · Pendu · Puzzle · Snake · Roue
// Memory séparé dans app-memory.js — Mars 2026
// Dépendances : app-core.js, app-account.js
// ═══════════════════════════════════════════════════════════
...
function puzzleShowPreview(){
  ...
  w.innerHTML='<img src="'+puzzleDataURLCurrent+'" style="width:120px;height:120px;border-radius:8px;object-fit:cover;margin-bottom:8px"><br><button id="puzzlePreviewCloseBtn" class="game-start-btn">Fermer</button>';
  var closeBtn = document.getElementById('puzzlePreviewCloseBtn');
  if(closeBtn) closeBtn.addEventListener('click', function(){ var pw=document.getElementById('puzzleWin'); if(pw) pw.style.display='none'; });
}
function puzzleWin(){
  var score=Math.max(0,1000-puzzleMoveCount*5);
  var win=document.getElementById('puzzleWin'); win.style.display='block';
  win.innerHTML='<div style="font-size:44px">🏆</div><div style="font-family:\'Playfair Display\',serif;font-size:18px;font-weight:700;color:var(--text);margin:8px 0">Puzzle terminé !</div><div style="font-size:13px;color:var(--sub)">'+puzzleMoveCount+' échanges · '+score+' pts</div><button id="puzzleWinReplayBtn" class="game-start-btn" style="margin-top:14px">Rejouer 🔀</button>';
  var replayBtn = document.getElementById('puzzleWinReplayBtn');
  if(replayBtn) replayBtn.addEventListener('click', function(){ puzzleReplay(); });
  if(puzzlePlayer){
...
(function(){
  ...
  var overlayHTML = '<div id="wheelOverlay" style="display:none;position:fixed;inset:0;z-index:2500;background:var(--bg);overflow-y:auto;-webkit-overflow-scrolling:touch;">'
    ...
    + '<div id="wheelBackBtn" style="width:36px;height:36px;border-radius:50%;background:var(--s2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg></div>'
  ...
  var _wheelBackBtn = document.getElementById('wheelBackBtn');
  if(_wheelBackBtn) _wheelBackBtn.addEventListener('click', function(){ closeWheelModal(); });
