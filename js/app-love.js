...
function spawnHeart(){
  ...
  if(!profile){
    console.warn('[❤️ DEBUG] spawnHeart: pas de profil sélectionné');
    return;
  }
  ...
  if(!coupleId){
    console.warn('[❤️ DEBUG] spawnHeart: couple_id est NULL ou invalide');
    return;
  }

  console.log('[❤️ DEBUG] spawnHeart: profile='+profile+', coupleId='+coupleId);
  ...
  if(numEl){
    var cur = parseInt((numEl.textContent||'0').replace('k','000').replace('M','000000')) || 0;
    numEl.textContent = fmtLikes(cur + 1);
    console.log('[❤️ DEBUG] Mise à jour optimiste: '+(cur+1)+' cœurs pour '+profile);
  }

  fetch(SB2_URL+'/rest/v1/rpc/increment_like_counter', {
    method:'POST',
    headers:sb2Headers({'Content-Type':'application/json'}),
    body:JSON.stringify({ p_profile: profile, p_couple_id: coupleId })
  })
  .then(function(r){ 
    console.log('[❤️ DEBUG] RPC increment_like_counter: status='+r.status);
    return r.ok ? r.json() : null; 
  })
  .then(function(data){ 
    console.log('[❤️ DEBUG] RPC response:', data);
    loadLikeCounters(); 
  })
  .catch(function(err){ 
    console.error('[❤️ DEBUG] Erreur RPC:', err);
    loadLikeCounters(); 
  });
}
}
...
function loadLikeCounters(){
  ...
  if(!coupleId){
    console.warn('[❤️ DEBUG] loadLikeCounters: couple_id est NULL');
    return;
  }

  console.log('[❤️ DEBUG] loadLikeCounters: coupleId='+coupleId);

  fetch(SB2_URL+'/rest/v1/v2_like_counters?couple_id=eq.'+coupleId+'&select=profile,total', {
    headers: sb2Headers()
  })
  .then(function(r){ 
    console.log('[❤️ DEBUG] loadLikeCounters: status='+r.status);
    return r.ok ? r.json() : []; 
  })
  .then(function(rows){
    console.log('[❤️ DEBUG] loadLikeCounters: rows=', rows);
    if(!Array.isArray(rows)) return;
    rows.forEach(function(r){
      ...
      if(r.profile==='girl' && elGirl){
        ...
        console.log('[❤️ DEBUG] Compteur girl: '+r.total);
      }
      if(r.profile==='boy'  && elBoy){
        elBoy.textContent  = fmtLikes(r.total);
        console.log('[❤️ DEBUG] Compteur boy: '+r.total);
      }
    });
  }).catch(function(err){
    console.error('[❤️ DEBUG] Erreur loadLikeCounters:', err);
  });
}
