...
(function(){
    ...
    if(name === 'chat'){
      ...
      if(center) center.innerHTML =
        '<div class="dm-topbar-avatars">' +
          '<div class="dm-avatar dm-avatar-girl" style="background:none;overflow:hidden;">' + (window.yamAvatarHTML ? window.yamAvatarHTML('girl') : '') + '</div>' +
          '<div class="dm-avatar dm-avatar-boy" style="background:none;overflow:hidden;">' + (window.yamAvatarHTML ? window.yamAvatarHTML('boy') : '') + '</div>' +
        '</div>' +
  ...
  function doSend(){
    ...
    if(!text) return;
    input.value = '';
    input.blur(); // ✅ FIX — ferme le clavier sur mobile après envoi
    updateSendBtn();
...
(function(){
  ...
  window.showMsgHeaderPill = function(emoji, name, text){
    ...
    // ✅ FIX — Utiliser la vraie photo du partenaire si disponible
    if(av){
      if(window._yamPartnerAvatarUrl){
        av.innerHTML = '<img src="' + window._yamPartnerAvatarUrl + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">';
      } else {
