...
(function(){
  ...
  function fetchMsgs(){
    ...
    .then(function(rows){
      ...
      rows.forEach(function(msg){
        ...
        // Ne marquer comme lu QUE si on est vraiment dans la conversation (dmChatScreen visible)
        var chatScreen = document.getElementById('dmChatScreen');
        if(chatScreen && chatScreen.style.display !== 'none' && msg.sender !== identity && !msg.seen){
          markSeen(msg.id);
        }
      });
  ...
  window.openHiddenPage = function(){
    ...
    if(saved === 'girl' || saved === 'boy'){
      ...
      showScreen('home');
      // Afficher directement la carte conv (pas le logo)
