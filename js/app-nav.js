...
(function(){
  ...
  window.yamSwitchTab = function(tab) {
    ...
    // Vérifier le verrou "nous" à l'entrée sur l'onglet
    // ← intégré depuis app-inline.js BLOC 7
    if(tab === 'nous'){
      setTimeout(function(){
    ...
    // Sous-titre du header principal selon l'onglet
    // ← intégré depuis app-inline.js BLOC 9
    (function(){
      var el = document.getElementById('hdrSub');
      if(el) el.textContent = (tab === 'musique') ? 'Notre univers en musique' : 'Notre univers';
    })();

    // Sync titre sticky header
    // ← intégré depuis app-inline.js BLOC 8
    if(typeof window._yamStickySetTab === 'function') window._yamStickySetTab(tab);

    // Notifier les modules abonnés (cowatch, etc.) sans patch de yamSwitchTab
    document.dispatchEvent(new CustomEvent('yam:tab_switched', { detail: { tab: tab } }));
  };
...
(function(){
  ...
  (function(){
    ...
    s.textContent =
      ...
      /* ── Nav lobby pulse — 3 couleurs ── */
      '@keyframes yamNavPulse{0%,100%{opacity:1;transform:scale(1);}50%{opacity:.6;transform:scale(.93);}}' +
      '#navJeux.partner-in-lobby--gold{animation:yamNavPulse 1.4s ease-in-out infinite;}' +
      '#navJeux.partner-in-lobby--gold svg,#navJeux.partner-in-lobby--gold span:not([id]){color:#f59e0b!important;fill:#f59e0b!important;}' +
      '#navJeux.partner-in-lobby--pink{animation:yamNavPulse 1.4s ease-in-out infinite;}' +
      '#navJeux.partner-in-lobby--pink svg,#navJeux.partner-in-lobby--pink span:not([id]){color:#f472b6!important;fill:#f472b6!important;}' +
      '#navJeux.partner-in-lobby--green{animation:yamNavPulse 1.4s ease-in-out infinite;}' +
      '#navJeux.partner-in-lobby--green svg,#navJeux.partner-in-lobby--green span:not([id]){color:#22c55e!important;fill:#22c55e!important;}';
...
// ═══════════════════════════════════════════════════════════════════
// Sticky header universel
// ← migré depuis app-inline.js BLOC 8
// Expose window._yamStickySetTab(tab) — appelé directement par yamSwitchTab
// ci-dessus, sans patch. Plus de monkey-patch yamSwitchTab ici.
// ═══════════════════════════════════════════════════════════════════
(function(){
  var TAB_TITLES = {
    home:    'Accueil',
  ...
  function _getActiveScrollY() {
    ...
    if(panelId){
      ...
      if(panel){
        ...
        if(cs.overflowY === 'auto' || cs.overflowY === 'scroll') return panel.scrollTop;
      }
    }
  ...
  function applyProgress(progress){
    ...
    if(mainHeader){
      ...
      mainHeader.style.transform     = 'translateY(' + (-progress * _mainHeaderH) + 'px)';
  ...
  // Exposé et appelé directement par yamSwitchTab — plus de patch
  window._yamStickySetTab = function(tab){
    var tabIds = { home: 'yamHomeTab', nous: 'yamNousTab', jeux: 'yamJeuxTab', musique: 'yamMusiqueTab' };
    ...
    if(prevPanelId){
      ...
      if(prevPanel){
        ...
        if(cs.overflowY === 'auto' || cs.overflowY === 'scroll') prevPanel.scrollTop = 0;
      }
    }
    currentTab = tab;
    resetToHidden();
  ...
  function init(){
    ...
    // Engrenage sticky → ouvre Paramètres
    setTimeout(function(){
