...
setTimeout(function(){
  ...
  document.addEventListener('touchend', function(e){
    ...
    function doSwipeBack(){
      if(sc === 'conv'){
        // Conv = premier écran réel → fermer directement
        if(window.closeHiddenPage) window.closeHiddenPage();
      } else if(sc === 'chat'){
        // Chat → retour vers conv directement, sans passer par history/pile
        // history.go(-1) causait une page blanche via applyTop/_dmRawShowChat
        window._yamSwipeInProgress = true;
        if(window._dmRawShowConv) window._dmRawShowConv('backward');
        else if(window.dmShowConv) window.dmShowConv();
        // Sync la pile sans ré-animer
