...
function _memShowScreen(id) {
  ['memScreenLobby','memScreenMode','memScreenClassic','memScreenEcho','memScreenArchi','memScreenHanabi','memScreenMind'].forEach(function(s) {
    var el = _memEl(s);
...
function _memStartLobby() {
  ...
  _memMp = YAMMultiplayer.init({
    ...
    onPresenceUpdate: function(isOnline) {
      ...
      _mindUpdatePresenceDot(isOnline);
    },
...
function _memGoToModeSelect(gameRow) {
  ...
  ['classic','echo','archi','all','hanabi','mind'].forEach(function(mode) {
...
function _memVoteMode(mode) {
  ...
  ['classic','echo','archi','all','hanabi','mind'].forEach(function(m) {
...
function _memUpdateVotes(state) {
  var labels = {classic:'Classique+', echo:'Écho', archi:'Architecte', all:'ALL', hanabi:'Hanabi', mind:'The Mind'};
...
function _memLaunchSingle(mode, gameRow) {
  ...
  else if (mode === 'mind')   _mindStart(gameRow);
}
...
function _hanabiConfirmPlay() {
  ...
  if (correct) {
    ...
    if (card.num === 5 && ns.blue_tokens < 15) ns.blue_tokens++;
...
function _hanabiConfirmDiscard() {
  ...
  var card = ns[myHandKey][_hanabiSelectedCard];
  ns.discard = (ns.discard||[]).concat([card]);
  if (ns.blue_tokens < 15) ns.blue_tokens++;
...
function _mindBuildState(level) {
  ...
  var n = level;
  // Les deals sont stockés dans le state partagé —
  // chaque joueur lit son propre deal et upsert sa propre ligne mind_hands
  var girlCards = deck.slice(0, n).sort(function(a,b){return a-b;});
  var boyCards  = deck.slice(n, n*2).sort(function(a,b){return a-b;});
  return {
    phase:       'mind',
    mode:        'mind',
    level:       level,
...
// ── Écrire SA PROPRE main dans mind_hands ──
// Chaque joueur upsert uniquement sa ligne (RLS enforce this)
function _mindWriteMyHand(coupleId, level, cards) {
  return _mindUpsert({ couple_id: coupleId, role: _memProfile, level: level, cards: cards });
}
...
function _mindStart(gameRow) {
  ...
  // Chaque joueur lit son deal depuis le state partagé et upsert sa propre ligne
...
function _mindSelectCard(idx) {
  ...
  if (playBtn) {
    ...
    playBtn.classList.toggle('mnd-play-btn--ready', _mindSelected !== null);
  }
}
...
// ── Banner niveau ──
function _mindShowLevelBanner(level, isStart) {
  var banner = _memEl('memMindLevelBanner');
  var numEl  = _memEl('memMindLevelBannerNum');
  var cdEl   = _memEl('memMindLevelBannerCd');
