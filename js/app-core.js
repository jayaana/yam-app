...
// ═════════════════════════════════════════════════════════════════
// v2ApplyDynamicNames — Noms des partenaires dans toute l'UI
// ← migré depuis app-inline.js BLOC 6
// Appelé après login (yam:session_ready) et au retour sur home.
// ═════════════════════════════════════════════════════════════════
...
function v2ApplyDynamicNames() {
  ...
  gameGirlBtns.forEach(function(id) {
    ...
    var icon = el.querySelector('.gender-select-icon');
    el.textContent = ''; if (icon) el.appendChild(icon); el.appendChild(document.createTextNode(girlName));
  });
  gameBoyBtns.forEach(function(id) {
    var el = document.getElementById(id); if (!el) return;
    var icon = el.querySelector('.gender-select-icon');
    el.textContent = ''; if (icon) el.appendChild(icon); el.appendChild(document.createTextNode(boyName));
  });
  ...
  if (prankLockTitle) prankLockTitle.textContent = 'Accès bloqué par ' + (u.partner_pseudo || 'ton partenaire');
}
window.v2ApplyDynamicNames = v2ApplyDynamicNames;

// Déclencher après login et après setProfile
document.addEventListener('yam:session_ready', function() {
  setTimeout(v2ApplyDynamicNames, 200);
});
...
var _subviewIds = ['gamesView','memoryView','penduView','puzzleView','snakeView','skyjoView','ochoView','quizView','hiddenPage','prankMenu','bookView'];
