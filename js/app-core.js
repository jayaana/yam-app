...
function sb2Headers(extra){
  var u = v2GetUser ? v2GetUser() : null;
  var token = (u && u.token) ? u.token : null;
  // Récupère le token depuis la session localStorage si disponible
  try {
    if(!token){
...
(function(){
  ...
  if(saved === 'light' && !document.body.classList.contains('light')){
    ...
    document.addEventListener('DOMContentLoaded', function(){
      ...
      if(btn) btn.textContent = '🌙 Thème';
      var fBtn = document.getElementById('floatingThemeBtn');
      if(fBtn) fBtn.textContent = '🌙'; // Icône seule
      var hBtn = document.getElementById('themeToggleHome');
      if(hBtn) hBtn.textContent = '🌙 Thème';
      ['qz','gv','dm','pm','home'].forEach(function(prefix){
        var moon = document.getElementById(prefix+'ThemeIconMoon');
        var sun  = document.getElementById(prefix+'ThemeIconSun');
