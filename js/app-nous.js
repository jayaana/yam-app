...
(function(){
  ...
  function _setupDoubleTapOnPhotos(section, profile){
    ...
    SLOTS.forEach(function(slot){
      ...
      [imgEl, emptyEl].forEach(function(el){
        if(!el) return;
        if(el._doubleTapSetup) return;
        el._doubleTapSetup = true;
        var _lastTap = 0;
        ...
        // Support souris desktop (double-clic)
  ...
  window.slotEditPhotoClick = function(){
    var inp = document.getElementById('slotEditPhotoInput'); if(inp){ inp.value=''; inp.click(); }
  };

  window.slotEditHandlePhoto = function(input){
    ...
    reader.onload = function(e){
      var photoDiv = document.getElementById('slotEditPhoto');
      var placeholder = document.getElementById('slotEditPhotoPlaceholder');
      if(photoDiv){ photoDiv.style.backgroundImage = 'url('+e.target.result+')'; }
      if(placeholder) placeholder.style.display = 'none';
    };
    reader.readAsDataURL(file);
    // Upload via la fonction existante de la section
    if(_slotEditSection === 'elle' && typeof window.elleUploadFile === 'function'){
      window.elleUploadFile(_slotEditSlot, file);
    } else if(_slotEditSection === 'lui' && typeof window.luiUploadFile === 'function'){
      window.luiUploadFile(_slotEditSlot, file);
    }
  };
