  ...
  function _setupDoubleTapOnPhotos(section, profile){
    ...
    SLOTS.forEach(function(slot){
      ...
      [imgEl, emptyEl].forEach(function(el){
        if(!el) return;
        if(el._doubleTapSetup) return; // éviter double init
        ...
        el.addEventListener('touchend', function(e){
          ...
          if(now - _lastTap < 350){
            e.preventDefault();
            if(section === 'elle') window.elleEditDesc && window.elleEditDesc(slot);
            else window.luiEditDesc && window.luiEditDesc(slot);
          }
          _lastTap = now;
        }, {passive: false});
      });
